...
export default function IconResizer(
	moduleOptions: IconResizerPluginOptions,
): PluginOption {
	return {
		name: "icon-generator",
			...
			await moduleOptions.variants.map((iconConfig): Promise<unknown> => {
				if (!iconConfig.src) {
					return new Promise((_resolve, reject) =>
						reject(new Error("No source file specified")),
					);
				}
				...
				console.info(
					`Generating ${iconConfig.src} into sizes [${moduleOptions.sizes.join(", ")}] -> ${moduleOptions.outputFolder}`,
				);

				const promises = [];
				for (const size of moduleOptions.sizes) {
					const outputFileName = `${iconConfig.prefix}${size}.png`;
					let basePromise = sharp(join(config.root, iconConfig.src)).resize(
						size,
					);

					if (iconConfig.bgColor) {
						basePromise = basePromise.flatten({
