...
<template>
  <popup-sheet class="fixed w-full max-w-2xl md:p-4" open>
    <div class="flex flex-col px-6 pt-6 pb-6 transition bg-surface-light dark:bg-surface-dark md:backdrop-blur-xl md:backdrop-brightness-200 md:backdrop-contrast-50 md:bg-surface-light/80 md:dark:bg-surface-dark/80 shadow-lg md:pb-3 rounded-t-2xl md:rounded-2xl max-h-[90vh] md:h-auto md:min-h-[24rem] ring-1 ring-surface-onlight/20 dark:ring-surface-ondark/20 md:ring-inset">
      <UiProgress :progress="state.page / 3" class="mb-4 flex-shrink-0" />
      ...
      <div class="flex flex-col gap-2 mt-4 md:flex-row">
        ...
        <Button v-else-if="state.page < 3" class="flex-grow w-full" default-style :importance="ButtonImportance.Filled" @click="state.page += 1">
