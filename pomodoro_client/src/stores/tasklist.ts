	...
	actions: {
		newTask(
			...
			const newTask: Task = {
				...
				id:
					this.tasks.reduce(
						(lastMax, currentItem) => Math.max(currentItem.id ?? 0, lastMax),
						-1,
					) + 1,
			};
		...
		changePriority(item: Task, direction: TaskMoveDirection) {
			...
			if (itemIndex >= 0) {
				...
				this.tasks[itemIndex].priority = Math.min(
					5,
					Math.max(-5, this.tasks[itemIndex].priority + Math.sign(direction)),
				);
			}
		},
		...
		toggleWorking(item: Task) {
			const itemIndex = this.tasks.indexOf(item);
			if (itemIndex >= 0) {
				this.tasks[itemIndex].state =
					this.tasks[itemIndex].state === TaskState.inProgress
						? TaskState.waiting
		...
		setComplete(id: number, complete: boolean) {
			...
			this.tasks[itemIndex].state = complete
				? TaskState.complete
				: TaskState.inProgress;
		},
		...
		removeCompleted() {
			this.tasks = this.tasks.filter(
				(task) => task.state !== TaskState.complete,
			);
		},

		moveItem(item: Task, newIndex: number) {
			const oldIndex = this.tasks.indexOf(item);
			if (oldIndex < 0 || newIndex >= this.tasks.length) {
				return;
			}
