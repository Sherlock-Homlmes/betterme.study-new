...
export default function VitePWAGenerator(
	moduleOptions: PWAModuleOptions,
): PluginOption {
	...
	return {
		...
		async configResolved(config) {
			...
			await generateSW({
				...
				globPatterns: ["**/*.{js,json,mjs,css,html,svg,woff2}"],
				sourcemap: false,
				navigationPreload: true,
				runtimeCaching: [
					{
						urlPattern: ({ url }) =>
							/\.(js|json|css)$/.exec(url.pathname) !== null,
						handler: "CacheFirst",
						options: {
							cacheName: "code",
							cacheableResponse: {
					...
					{
						urlPattern: ({ url }) =>
							/\.(jpg|png|jpeg|svg)$/.exec(url.pathname) !== null,
						handler: "CacheFirst",
						options: {
							cacheName: "assets",
							cacheableResponse: {
					...
					{
						urlPattern: ({ url }) =>
							["", "/", "/index", "/index.html"].includes(url.pathname),
			...
			console.info(
				`Generated service worker at ${path.join(outputDir, moduleOptions.swPath)}`,
			);
		},
	};
}
