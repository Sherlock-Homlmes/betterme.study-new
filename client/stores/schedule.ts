...
export const useSchedule = defineStore("schedule", {
	...
	getters: {
		...
		getSchedule(state): ScheduleEntryComplete[] {
			...
			const returnArray =
				state.items.length === 0 ? [] : JSON.parse(JSON.stringify(state.items));
			...
			for (let index = 0; index < returnArray.length; index++) {
				...
				const itemType: keyof Settings["schedule"]["lengths"] = returnArray[
					index
				].type
					? returnArray[index].type
					: scheduleTypes[returnArray[index].id % scheduleTypes.length];
	...
	actions: {
		...
		advance() {
			...
			this.items.push(
				createScheduleEntry(
					this.items.length > 0 ? this.items.slice(-1)[0].id + 1 : 0,
				),
			);
		},
